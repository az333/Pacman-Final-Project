  ...
  for (int r = 0; r < maze.length; r ++) { 
    for (int c = 0; c < maze[r].length; c ++) { 
      if (maze[r][c].isValid() && maze[r][c].hasDot()) { 
        fill(#FFEC00);
        noStroke();
        ellipse(maze[r][c].getC() * 16 + 8, maze[r][c].getR() * 16 + 8, 8, 8);
      }
    }
...
void keyPressed() {
  ...
  if (key == CODED) {
    ...
    if (keyCode == UP) {
      while (n.getR() > 0 && maze[n.getR() -1][n.getC()].isValid()) 
      //int roww = n.getR();
